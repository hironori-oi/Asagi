...
/**
 * 全体 UI のオーバーレイ表示状態（AS-114 / AS-116 / AS-120 / AS-121 / AS-UX-05）。
 *
 * - commandPalette: Ctrl+K で開く shadcn/cmdk Command palette
 * - settings: 設定モーダル（vaul Drawer）
 * - help: キーバインド一覧モーダル
 * - sidebarActiveTab: AS-UX-05 / DEC-018-037 §① 5-tab Sidebar の選択タブ。
 *   localStorage に persist し、再起動後も復元する。
 * - sidebarCollapsed: AS-UX-05 / Sumi DEC-082 翻訳。Cmd+B で 240px <-> 48px トグル。
 *
 * グローバル state にすることでキーバインドフックからもメニューからも開閉可能にする。
 */
export type SidebarTab = 'sessions' | 'files' | 'runtime';
